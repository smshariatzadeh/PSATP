...
if length(k) > length(h)
  ...
  for i = 1:length(k)
    ...
    con(k(i),4) = con(k(i),4) + u(i)*a.con(i,4)*a.con(i,2)/Settings.mva;
    con(k(i),5) = a.con(i,5)*vb;
    con(k(i),6) = con(k(i),6) + u(i)*a.con(i,6)*a.con(i,2)/Settings.mva;
    con(k(i),7) = con(k(i),7) + u(i)*a.con(i,7)*a.con(i,2)/Settings.mva;

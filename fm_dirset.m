...
switch type
 ...
 case 'formatlist'
  ...
  formati{PSATSLX} = 'PSAT Simulink (.slx)';
  formati{PST} = 'PST (.m)';
 ...
 case 'chformat'
  ...
  switch int32(formato)
   ...
   case PSATSLX, file = 'simulinkSLX';  %add by majid (SLX avatar)
  ...
  if formato == PSATMDL  ||  formato == PSATSLX
    set(hdlp,'Enable','on')
    set(hdlp,'Visible','on')
  else
    set(hdlp,'Enable','off')
 ...
 case 'convert'
  ...
  for i = 1:length(numfile)
    ...
    switch get(hdl,'Value')
     ...
     case PSATSLX          %edit By S.M. Shariatzadeh
      first = double(filename(1));   %edit S.M. Shariatzadeh
      if first <= 57 && first >= 48  %edit  S.M. Shariatzadeh
        copyfile(filename,['d',filename])  %edit S.M. Shariatzadeh
        filename = ['d',filename]; %edit S.M. Shariatzadeh
        fm_disp(['Use modified file name <',filename,'>']) %edit
      end  %edit S.M. Shariatzadeh
      check = sim2psat(filename,pwd); %edit S.M. Shariatzadeh
 ...
 case 'openfile'
  ...
  switch type
   case {PSAT,PSATPERT,PSATMDL,PSATSLX,VITRUVIO}   %edit by majid
    % nothing to do!
   case PCFLO
 ...
 case 'preview'
  ...
  hdl = findobj(Fig.dir,'Tag','PopupMenu1');
  type = get(hdl,'Value');
  if type ~= PSATMDL &&  type ~= PSATSLX  % edit by majid
    cd(Path.local)
    return
  end
